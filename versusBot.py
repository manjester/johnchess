...
while run:
    for event in pygame.event.get():
        ...
        if turn % 2 == 0:
            if not gameend:
                pygame.display.set_caption("John Chess: White to play")
            else:
                pygame.display.set_caption("John Chess: GAME OVER. Black wins!")
                endscreen("black")
        elif turn % 2 == 1:
            if not gameend:
                pygame.display.set_caption("John Chess: Black to play")
            else:
                pygame.display.set_caption("John Chess: GAME OVER. White wins!")
                endscreen("white")
        ...
        if turn % 2 == 1: # black turn
            ...
            drawPossible(possibleMoves)
            if possibleMoves != []:
                moveto = random.choice(possibleMoves)
                blackCoords[selection] = moveto
                if moveto in whiteCoords:
                    white_piece = whiteCoords.index(moveto)
